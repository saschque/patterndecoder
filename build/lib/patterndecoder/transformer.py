...
Example:
    ...
    transformer = TransformerBlock(
        window_params={"batch_size": 32, "observation_window_size": 60, ...},
...
from transformer_models.embedding import TokenEmbedding, SinusoidalPositionalEncoding
...
class TransformerBlock(tf.keras.Model):
    ...
    def __init__(self, window_params, transformer_params, training_params, attn, **kwargs):
        ...
        Args:
            window_params (dict): Dictionary containing window configuration parameters:
                - batch_size: Batch size for input tensors
                - observation_window_size: Length of input sequences
                - n_features: Number of input features
                - output_size: Size of the output (prediction horizon)
            transformer_params (dict): Dictionary containing transformer architecture parameters:
            ...
            training_params (dict): Dictionary containing training configuration parameters:
                - dropout: Dropout rate
                - l1_reg: L1 regularization coefficient
                - l2_reg: L2 regularization coefficient
            attn (dict): Dictionary containing attention mechanisms:
                - enc_attn: Attention mechanism for encoder
                - dec_attn: Attention mechanism for decoder (if different from encoder)
        ...
        self.inputs = tf.keras.Input(
            shape=(
                window_params["batch_size"],
                window_params["observation_window_size"],
                window_params["n_features"]),
            name="inputs"
        )
        ...
        self.encoder = Encoder(
            ...
            transformer_params["n_layers_enc"],
            attn,
        ...
        kernel_regularizer = tf.keras.regularizers.l1_l2(
            l1=training_params["l1_reg"],
            l2=training_params["l2_reg"],
        )
        ...
        self.output_dense = tf.keras.layers.Dense(
            units = window_params["output_size"],
