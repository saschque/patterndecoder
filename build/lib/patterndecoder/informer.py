...
from transformer_models.transformer import EncoderLayer as TransformerEncoderLayer
from transformer_models.transformer import Encoder as TransformerEncoder
...
class InformerBlock(TransformerBlock):
    ...
    Inherits all parameters from TransformerBlock:
        ...
        transformer_params (dict): Architecture parameters including d_model, 
            units, n_layers_enc, n_layers_dec, and activation.
        training_params (dict): Training configuration including dropout, 
            l1_reg, and l2_reg.
        attn (dict): Attention mechanisms with enc_attn (ProbSparse recommended) 
            and dec_attn for encoder and decoder respectively.

    Note:
        The encoder uses ProbSparse attention and distilling layers that 
        progressively reduce sequence length, making it efficient for 
        long sequence forecasting tasks.
    """
    def call(self, inputs, training=True):
        """
        ...
        Args:
            inputs (tf.Tensor): Input time series tensor of shape 
                (batch_size, observation_window_size, n_features).
            training (bool): A boolean indicating whether the layer is in training mode.
                Defaults to True.
